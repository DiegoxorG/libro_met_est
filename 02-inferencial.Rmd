...
```{r inf-cor-dispersion, fig.width=10, fig.height=6}
pacientes %>%
...
Las pruebas anteriores comparan promedios o miden correlaciones. La prueba chi-cuadrado de independencia responde otra pregunta: si **dos variables categóricas** están asociadas. Como `sex` es la única categórica del conjunto de datos, seguimos el procedimiento visto en clase y construimos las demás categorizando variables numéricas: el grupo de edad ya está definido, y la severidad motora se clasifica en alta o baja según el promedio de `motor_UPDRS` de cada paciente esté por encima o por debajo de la mediana. La pregunta principal es si el nivel de severidad está asociado con el grupo de edad; como segunda tabla, se cruza con el sexo.
...
```{r inf-chi-grafico, fig.height=4}
pacientes %>%
...
Cálculo manual como en clase, comparado con `chisq.test`:
...
cramer_v(tabla_edad_severidad, correct = FALSE)
```

V = 0.27, un efecto entre pequeño y medio, coherente con el eta cuadrado de 0.13 del ANOVA: la asociación entre edad y severidad existe en los datos, pero la muestra es pequeña para que resulte significativa.
